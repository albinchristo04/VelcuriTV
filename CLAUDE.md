...
- **`app/`** — Android app (Gradle, Kotlin, Android Application plugin). Module name in `settings.gradle.kts` is `:app`. Application ID `com.velcuri.tv`. Namespace `com.velcuri.tv`. Older source still uses `com.lagradost.cloudstream3` imports — both packages coexist because `library/` publishes under `com.lagradost.api`.
...
./gradlew :app:testPrereleaseDebugUnitTest --tests "com.velcuri.tv.MyTest.method"   # single test
